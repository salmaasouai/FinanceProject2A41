<?php
 ...
 if (isset($_POST['query'])) {
  ...
  $tab = $ec->rechercher($query);
  /*header('Content-Type: application/json');*/
    echo json_encode($results);
} 
else 
{
  // Si aucune requête de recherche n'est soumise, afficher toutes les données
  if(isset($_GET['sort']) && $_GET['sort'] == 'participants') {
    // Tri des événements par nombre de participants
    $sql = "SELECT * FROM events ORDER BY NbParticipants DESC";
} else {
    // Requête par défaut sans tri
...
$liste = $db->query($sql);
 /* header('Content-Type: application/json');*/
  ...
  // Vérifiez si le paramètre de tri est présent dans l'URL
      ...
      <div class="card">
        <div class="card-body">
          ...
          <table id="eventTable" class="table datatable">
                <thead>
                  <tr>
                    ...
                    <th scope="col">NbParticipants</th>
                    <th scope="col">detailsEvent</th>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><?= $event['detailsEvent']; ?></td>
                    <td><?= $event['categorie']; ?></td>
                    <td><img src="events_pics/<?= $event['ImgEv']; ?>" alt="Event Image" width="100" height="100"></td>
                    <td>
                      <a href="deleteEv.php?id=<?= $event['IdEv']; ?>" class="btn btn-danger">Delete</a>
                      <a href="updateEv.php?id=<?= $event['IdEv']; ?>" class="btn btn-primary">Update</a>
                    </td>
                  </tr>
                  <?php endforeach; ?>
                  <!-- End PHP Loop -->
          ...
          <div class="text-center">
            <a href="pdfEv.php" class="btn btn-primary">Generate PDF</a>
            <a href="AddEv.php" class="btn btn-secondary">Go back</a>
            <a href="afficherEv.php?sort=participants"class="btn btn-primary">Trier par Nombre de Participants</a>
...
<script>
    ...
    document.getElementById("sortBtn").addEventListener("click", function() {
        ...
        xhr.open("POST", "sortEvents.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                // Mettez à jour le tableau avec les données triées reçues en réponse
                var data = JSON.parse(xhr.responseText);
                // Code pour mettre à jour le tableau avec les données triées
            }
        };
        xhr.send(); // Envoyer la requête de tri
    });
});
