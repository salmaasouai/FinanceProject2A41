...
<a class="nav-link collapsed" href="pages-login.php">
 ...
 <span>Login</span>
  ...
  <!-- End General Form Elements -->
    ...
    let myinput = document.getElementById("Titre");
    let titre = myinput.value.trim();

    let titreExists = eventC.geteventByTitre(titre); // Vérifier si le titre existe déjà

    if (titreExists) {
        let myerror = document.getElementById("error1");
        myerror.innerHTML = "Le titre existe déjà";
        myerror.style.color = "red";
        e.preventDefault(); // Empêcher la soumission du formulaire si le titre existe déjà
    }
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            if (xhr.responseText === 'exists') {
                ...
                // Le titre n'existe pas encore, soumettre le formulaire
                myform.submit();
            }
        }
    };
